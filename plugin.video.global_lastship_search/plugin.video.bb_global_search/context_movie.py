...
def search_movie(title):
    try:
        if not title:
            log("No title provided for movie search", xbmc.LOGDEBUG)
            return

        clean_title = clean_search_title(title)
        log(f"Cleaned movie title: {clean_title}", xbmc.LOGDEBUG)
        ...
        log(f"Lastship movie search executed: {lastship_url}", xbmc.LOGDEBUG)

    except Exception as e:
        log(f"Error starting movie search: {e}", xbmc.LOGERROR)
        xbmcgui.Dialog().notification(
            "Film suchen",
...
def main():
    try:
        ...
        if not title:
            log("No title found for movie search", xbmc.LOGDEBUG)
            return

        search_movie(title)

    except Exception as e:
        log(f"Error in movie context main: {e}", xbmc.LOGERROR)
        xbmcgui.Dialog().notification(
            "Film suchen",
            f"Fehler: {str(e)}",
            xbmcgui.NOTIFICATION_ERROR,
